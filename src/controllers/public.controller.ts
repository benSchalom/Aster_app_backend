...
const css = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;
         background: #f5f5f5; min-height: 100vh; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 20px; }
  ...
  .erreur { background: #FEF2F2; color: #E24B4A; padding: 12px 16px; border-radius: 10px; font-size: 14px; margin-bottom: 16px; }
  .footer { text-align: center; margin-top: 20px; font-size: 12px; color: #bbb; }
  .centre { text-align: center; }
`

/**
 * GET /p/:programmeId — formulaire d'enrollment
 */
            ...
            <div class="card">
                ...
                <form method="POST" action="/p/${programmeId}">
                    ...
                    <input type="text" id="clientNom" name="clientNom"
                           placeholder="Nom et prenom" required minlength="2" autocomplete="name">
                    <label for="clientTelephone">Votre telephone</label>
                    <input type="tel" id="clientTelephone" name="clientTelephone"
                           placeholder="Ex: 5141234567" required minlength="7" autocomplete="tel">
...
/**
 * POST /p/:programmeId — traitement de l'inscription
 */
        ...
        if (programme.type === 'points' || programme.type === 'gratuite') {
            ...
            const carte = await prisma.carte.create({
                data: {
                    numeroSerie,
                    programmeId,
                    commercantId: programme.commercantId,
                    clientNom: nom,
            ...
            // Abonnement — creer une demande, montrer page d'attente
            const demande = await demandeService.creer(programmeId, nom, telephone)

            // Si une demande existe deja, obtenirStatut via l'ID existant
...
const pageAttente = (demandeId: string, base: string): string => `
    <!DOCTYPE html><html lang="fr">
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        ...
        <style>
            ...
            .spinner { width: 48px; height: 48px; border: 4px solid #e0e0e0;
                       border-top-color: #3B9EE8; border-radius: 50%;
...
/**
 * GET /carte/:carteId — page publique de la carte (sans nom du client)
 */
export const pageCarte = async (req: Request, res: Response) => {
    ...
    const labelEtat = etatLabel(p.type, p.valeur, carte.etat)
...
/**
 * GET /api/public/demande/:demandeId/statut — polling statut (sans auth)
 */
